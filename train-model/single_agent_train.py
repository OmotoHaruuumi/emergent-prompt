...
def args_define():
    ...
    parser.add_argument('--use_difffusion',type=bool,default=False,help='reconstruct image or not')
...
def main():
    ...
    if args.use_diffusion:
        vae = AutoencoderKL.from_pretrained(args.stable_diffusion_model_name)
        unet = UNet2DConditionModel.from_pretrained(args.stable_diffusion_model_name)
        vae.to(device)
        unet.to(device)
        noise_scheduler=DDPMScheduler.from_config(args.stable_diffusion_model_name)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for data in dataloader:
            ...
            if args.use_prior:
                ...
                loss_kld=0.0
            if args.use_diffusion:
                image_latent = vae.encode(data).latent_dist.sample()
                image_latent = image_latent * 0.18215

                noise = torch.randn_like(image_latent)
                bsz = image_latent.shape[0]    
                timesteps = torch.randint(0,noise_scheduler.num_train_timesteps,(bsz,),device=image_latent.device)
                ...
                target = noise_scheduler.get_velocity(image_latent,noise,timesteps)
        ...
        loss_history.append(loss/step_size)
